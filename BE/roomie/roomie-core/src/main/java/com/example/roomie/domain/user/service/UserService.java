...
public class UserService {
    ...
    public User join(SignUpRequestDto request){
        if(userRepository.existsByUid(request.getUid())) {
            throw new CustomException(null, ExceptionCode.DUPLICATE_ID);
        }
        if(userRepository.existsByEmail(request.getEmail())) {
            throw new CustomException(null, ExceptionCode.DUPLICATE_EMAIL);
        }
    ...
    public UserLoginResponseDto login(SignInRequestDto signInRequestDto){
        try {
            ...
            if (!passwordEncoder.matches(signInRequestDto.getPassword(), user.getPassword())) {
                throw new CustomException(null, ExceptionCode.INCORRECT_PASSWORD_REQUIRED);
            }
